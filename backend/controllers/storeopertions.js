...
exports.storedataget = (req, res) => {

    db.query(

        "SELECT s.Sid, s.sname, s.semail, s.saddress, AVG(r.Rdata) AS rating FROM store s LEFT JOIN rating r ON s.Sid = r.Sid GROUP BY s.Sid",
        (err, result) => {
            if (err) {
                console.log(err)
...
exports.SotreSearch = (req, res) => {
    console.log(req.body)

    const { sname, saddress } = req.body;

    db.query(
        "SELECT * FROM store WHERE sname = ? OR saddress = ?",
        [sname, saddress],
        (err, result) => {
