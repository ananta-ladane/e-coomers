...
const AdNav = () => {
    ...
    const getSearch = () => {
        ...
        if (role === "user") {
            setVuser(udata.find((u) => { return u.uname === data || u.uemail === data || u.upass === data }));
            console.log("get user page")
            setAlert(true)

        } else if (role === "store") {
            console.log("get store page")
            setVstore(sdata.find((s) => { return s.sname === data || s.semail === data || s.spass === data }))
            setSalert(true)

        } else {
            console.log("get admin page")
            setVadmin(addata.find((a) => { return a.Name === data || a.Email === data || a.Password === data }))
            setAalert(true)
        }
